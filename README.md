...
1. [mqtt_telemetry_uploader.yaml](mqtt_telemetry_uploader.yaml) publishes grouped telemetry, retained MQTT Discovery configs, and retained capability metadata.
2. [mqtt_command_receiver.yaml](mqtt_command_receiver.yaml) receives schema v2 JSON commands and dispatches Home Assistant service calls behind area/domain allowlists.
...
- Command schema compatibility controls (see the [v1 -> v2 Migration Guide](docs/migration-guide-v1-to-v2.md)):
...
Home Assistant, MQTT broker, and per-domain capability details are in the
